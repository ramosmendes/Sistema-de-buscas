...
def on_click(event):
    caminho.configure(state=NORMAL)
    caminho.delete(0, END)

    # faz a chamada durar apenas uma vez
    caminho.unbind('<Button-1>', on_click_id)
...
texto_caminho = Label(fr_quadro, text="Digite o caminho:",background='white',width=20,font=("Arial",18))
texto_caminho.grid(column=1, row=1)
...
caminho.insert(0, "Exemplo: /home/usr/jaja/")
caminho.configure(state=DISABLED)

# Rótulo de texto do termo adicionado
texto_termo = Label(fr_quadro, text="Digite o termo:",background='white',width=20,font=("Arial",18))
...
#id on_click
on_click_id = caminho.bind('<Button-1>', on_click)
